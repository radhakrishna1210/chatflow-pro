...
// Seconds-since-epoch `exp` out of a JWT, or null when it can't be read.
// Not a security check — the server verifies the signature — just enough to
// tell a live session from the remains of an old one without a network round trip.
...
// them to a login screen they never asked for.
function isAuthed() {
  ...
  if (alive(localStorage.getItem('refreshToken'))) {
    return true;
  }
...
export default function App() {
  ...
  // Legacy navigation API used by existing pages:
  // onNav('login'|'dashboard'|…)
...
function renderPage(path, nav, search) {
  ...
  if (
    ...
    return <Legal routePath={path} />;
  }

  // Product pages are public and stay public for signed-in visitors too:
  // the only redirect on this side of the guards is the landing page's,
  // and a logged-in user following a link to a feature page wants to read
  ...
  if (path === '/login') {
    return <Login onNav={nav} mode="login" />;
  }

  if (path === '/register') {
    return <Register onNav={nav} />;
  }
  ...
  if (path === '/setup') {
    ...
    // Guard effect redirects when appropriate.
