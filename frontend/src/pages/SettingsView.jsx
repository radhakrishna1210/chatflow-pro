    ...
    wFetch('/settings')
      ...
      .then(d => {
        if (d) {
          ...
          if (d.notifyNewConversation != null) {
            setNotifs({
              newConv: d.notifyNewConversation,
              tplApproved: d.notifyTemplateApproved,
              tplRejected: d.notifyTemplateRejected,
              campaignDone: d.notifyCampaignCompleted,
              highOptout: d.notifyHighOptout,
              rateLimitWarn: d.notifyRateLimit,
            });
          }

          setEmailNotifs(
            Object.fromEntries(
              EMAIL_NOTIF_OPTS.map(o => [
                o.id,
                d[o.id] != null ? d[o.id] : o.default,
              ])
            )
          );
        }
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '24px 28px', display: 'flex', flexDirection: 'column', gap: 16, maxWidth: 860 }}>
        ...
        {/* ── Business Profile ── */}
        <SectionCard icon="users" title="Business Profile">
