...
export default function CreateCampaign({ onBack }) {
  ...
  const handleGoLive = async () => {
    ...
    try {
      ...
      if (!recRes.ok) throw new Error(`recipients failed (${recRes.status}): ${await recRes.text()}`);
      const launchRes = await wFetch(`/campaigns/${campaign.id}/launch`, {
        method: 'POST', body: JSON.stringify({ scheduledAt }),
      });
      if (!launchRes.ok) throw new Error(`launch failed (${launchRes.status}): ${await launchRes.text()}`);
      onBack?.();
    } catch (err) {
  ...
  const handleSaveDraft = async () => {
    await wFetch('/campaigns', {
      method: 'POST',
      body: JSON.stringify({ name: campaignName || 'Untitled Draft', type: campaignType, numberId: selectedNumberId, templateId: selectedTemplateId, scheduleType, scheduledAt, status: 'DRAFT' }),
    }).catch(() => {});
  };
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {STEPS.map(s => (
            <StepWrap key={s.n} n={s.n} title={s.title} done={s.done} open={openStep === s.n} locked={isLocked(s.n)} onToggle={() => toggleStep(s.n)}>
              ...
              {s.n === 5 && <Step5 />}
              {s.n === 6 && <Step6 onRetryToggle={setRetriesActive} />}
              {s.n === 7 && <Step7 />}
