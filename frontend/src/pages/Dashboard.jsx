...
// Authentication's own analytics (OTP usage, verification rate, recent
// attempts) moved to the Authentication module — see AuthenticationDashboard.jsx.
// Campaign Analytics below now only ever fetches and renders regular
// campaigns, unchanged from how it already worked for that case.
  ...
  const generate = async (text) => {
    ...
    try {
      ...
      if (data.category === 'AUTHENTICATION') {
        setErr('Authentication (OTP) templates are created from the Authentication page, not here.');
        return;
      }
      setDraft(data);
      setWantImage(!!data.suggestImage);
    } catch (e) {
      setErr(e.message);
    } finally {
      setLoading(false);
...
const TemplatesView = () => {
  ...
  // Authentication templates are managed from the separate Authentication page
  // (/dashboard/authentication), not the normal Templates library — excluded
  // here rather than on the server so the Authentication API/DB logic stays
  // untouched.
