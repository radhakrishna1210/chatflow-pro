...
export default function PaymentsView() {
  ...
  const renderExpenses = () => (
    <div style={{ ...card, padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
      <h3 style={{ fontFamily: "'Syne',sans-serif", fontSize: 16, fontWeight: 700, color: 'var(--t1)' }}>Expense Logs</h3>
      <p style={{ fontSize: 13, color: 'var(--t2)' }}>Track real-time billing deductions for automation executions and WhatsApp template messages.</p>
      
      <div style={{ display: 'flex', flexDirection: 'column', border: '1px solid var(--bd)', borderRadius: 8, overflow: 'hidden' }}>
        {[
          { date: 'Just now', desc: 'AI Intent matching execution', amt: 0.20, type: 'DEBIT' },
          { date: '1 hour ago', desc: 'Voice AI receptionist inbound call (2 mins)', amt: 4.50, type: 'DEBIT' },
          { date: 'Today, 10:15 am', desc: 'Campaign delivery batch: "Product Update"', amt: 124.80, type: 'DEBIT' },
          { date: 'Yesterday, 04:30 pm', desc: 'WhatsApp Form Submission response routing', amt: 0.50, type: 'DEBIT' }
        ].map((item, idx, arr) => (
          <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 18px', borderBottom: idx < arr.length - 1 ? '1px solid var(--bd)' : 'none', background: 'rgba(255,255,255,0.01)' }}>
            <div>
              <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--t1)', marginBottom: 2 }}>{item.desc}</p>
              <p style={{ fontSize: 11, color: 'var(--t3)' }}>{item.date}</p>
            </div>
