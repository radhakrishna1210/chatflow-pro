...
// updated for new token: --green:#1EBF5E, --bg:#080B12, --surf:#0D1121
...
const LoginPage = ({ onNav }) => {
  ...
  const submit = e => {
    ...
    setStatus('loading');
    setTimeout(() => { setStatus('success'); setTimeout(() => onNav('dashboard'), 700); }, 1200);
  };
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', background: 'var(--bg)' }}>
      ...
      <div style={{
        ...
        background: 'linear-gradient(160deg,#07090F 0%,#0a0f1e 60%,#07090F 100%)',
        ...
        <div style={{ position:'relative', zIndex:1 }}>
          ...
          <h2 style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:'36px', color:'var(--t1)', marginBottom:'14px', lineHeight:1.1 }}>
            Welcome back
          </h2>
      ...
      <div style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', padding:'40px 28px' }}>
        <div style={{ width:'100%', maxWidth:'400px' }}>
          ...
          <h1 style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:'26px', color:'var(--t1)', marginBottom:'6px' }}>Sign in to your account</h1>
          <p style={{ fontSize:'14px', color:'var(--t2)', marginBottom:'32px' }}>
            Don't have an account?{' '}
            <span onClick={() => onNav('register')} style={{ color:'var(--green)', fontWeight:600, cursor:'pointer' }}>Create one free</span>
          </p>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:'14px', marginBottom:'22px' }}>
            ...
            <span style={{ fontSize:'12px', color:'var(--t3)', fontWeight:500 }}>or sign in with email</span>
            <div style={{ flex:1, height:'1px', background:'var(--bd)' }}/>
          </div>
          ...
          <form onSubmit={submit} style={{ display:'flex', flexDirection:'column', gap:'16px' }}>
            ...
            {status === 'success' && (
              <div style={{ padding:'10px 14px', borderRadius:'8px', background:'var(--gbg)', border:'1px solid var(--gbd)', fontSize:'13px', color:'var(--green)', display:'flex', alignItems:'center', gap:'8px' }}>
                <I n="check" s={14} c="var(--green)" w={2}/> Login successful — redirecting…
              </div>
            )}
            ...
            <button type="submit" disabled={status === 'loading' || status === 'success'}
              style={{
                width:'100%', padding:'13px', borderRadius:'9px', border:'none', cursor:'pointer',
                fontFamily:"'Plus Jakarta Sans',sans-serif", fontSize:'14px', fontWeight:700,
                color: status === 'success' ? 'var(--t1)' : '#07090F',
                background: status === 'success' ? 'var(--gbg)' : 'var(--green)',
                boxShadow: status === 'success' ? 'none' : 'var(--glow)',
                transition:'all .18s', opacity: status === 'loading' ? 0.7 : 1,
                display:'flex', alignItems:'center', justifyContent:'center', gap:'8px',
              }}>
              {status === 'idle' || status === 'error' ? <><I n="arrow" s={14} c="#07090F"/> Sign In</> : null}
              {status === 'loading' ? <><svg width="14" height="14" viewBox="0 0 14 14" style={{animation:'spin 1s linear infinite'}}><circle cx="7" cy="7" r="5" fill="none" stroke="#07090F" strokeWidth="2" strokeDasharray="20" strokeDashoffset="5"/></svg> Signing in…</> : null}
              {status === 'success' ? <><I n="check" s={14} c="var(--green)" w={2}/> <span style={{color:'var(--green)'}}>Done!</span></> : null}
            </button>
          </form>
...
// also expose icon helpers needed in auth for eye icon
const eyeIconDefs = {
