...
export async function list(req, res) {
  const templates = await templatesService.listTemplates(req.params.workspaceId);
  res.json(templates);
}
...
export async function syncFromMeta(req, res) {
  const result = await templatesService.syncTemplatesFromMeta(req.params.workspaceId);
  res.json(result);
}
...
export async function installLibrary(req, res) {
  const tpl = await templatesService.installFromLibrary(req.params.workspaceId, req.params.libId);
