...
//
// `countFailuresOnly` is what makes the allowance a guessing budget rather
// than a usage budget: signing in correctly costs nothing, so an office sharing
// one public IP is never locked out by its own successful logins.
//
...
// email verification at all, which made the whole OTP flow optional: anyone
// who skipped the UI and posted here got an account on an address they had
// never proved they owned. Account creation is /register/start +
// /register/verify, or Google OAuth (where Google has already verified the
// address).

// OTP-verified email signup
// Account is created only after the code is verified.
router.post(
  '/register/start',
  otpLimiter,
  validate({ body: authSchemas.signupStart }),
...
router.post(
  '/register/verify',
  loginLimiter,
  validate({ body: authSchemas.signupVerify }),
...
router.post(
  '/register/resend',
  otpLimiter,
  validate({ body: authSchemas.signupResend }),
  authController.resendSignupOtp
);

// OTP-verified password reset
// Same shape as signup: a code is emailed, the new password only takes
// effect once it's verified.
router.post(
  '/forgot-password',
  otpLimiter,
...
router.post(
  '/exchange',
  refreshLimiter,
  authController.exchangeOneTimeCode
);
...
router.get('/google/callback', (req, res, next) => {
  ...
  if (!statePayload) {
    console.warn(
      '[Google OAuth] Rejected callback with missing/invalid state (possible CSRF)'
    );
  ...
  passport.authenticate(
    ...
    (err, user) => {
      if (err) {
        ...
        console.error(
          `[Google OAuth] ${reason} — expected redirect_uri="${env.GOOGLE_CALLBACK_URL}". Check that this exact URL is in Google Cloud Console → Credentials → Authorized redirect URIs.`
        );

        console.error(
          '[Google OAuth] underlying error:',
          detail
        );

        return res.redirect(
          `${env.CLIENT_URL}/login?oauth_error=${encodeURIComponent(reason)}`
        );
      }

      if (!user) {
        return res.redirect(
          `${env.CLIENT_URL}/login?oauth_error=denied`
        );
      }

      req.user = user;

      return authController.googleCallback(
        req,
        res,
        next
      );
...
router.get(
  ...
  async (req, res) => {
    ...
    // Meta refuses a non-HTTPS redirect_uri outright, and only accepts one
    // that is registered on the app.
    if (!/^https:\/\//i.test(env.META_REDIRECT_URI || '')) {
      return res.status(503).json({
        error:
          'WhatsApp sign-in is not configured on this server: Meta requires an HTTPS redirect URI and ' +
...
router.get(
  ...
  async (req, res) => {
    ...
    // `detail` carries a short, safe excerpt of the real Meta error
    // when available so the user sees the actual reason instead of only
    // getting the generic `exchange_failed` code.
    const fail = (reason, detail) => {
      const url = new URL(
        `${env.CLIENT_URL}/dashboard/setup`
      );
    ...
    try {
      ...
      // Embedded Signup creates a WABA owned by the user.
      // Prefer the waba_id from the token exchange, else enumerate the
      // user's businesses.
      let wabaIds = [];

      if (shortTokenData.waba_id) {
        wabaIds = [
          shortTokenData.waba_id,
        ];
      }
      ...
      res.redirect(
        `${env.CLIENT_URL}/dashboard/setup?connected=true`
      );
    } catch (err) {
      console.error(
        '[Meta OAuth]',
...
// Exchanges the code for a long-lived token and stores the connection.
// The workspace is recovered from the signed OAuth `state` minted by
// POST /workspaces/:id/instagram/auth-url.
router.get(
  '/instagram/callback',
  instagramController.oauthCallback
);
