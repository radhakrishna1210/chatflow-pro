...
export const DEFAULT_CUSTOMIZATIONS = {
  ...
  ticket_customization: {
    stages: [
      { id: 'tstg_0', key: 'NEW', label: 'New', color: '#6366f1', isDefault: true, sortOrder: 0 },
      { id: 'tstg_1', key: 'OPEN', label: 'Open', color: '#3b82f6', isDefault: false, sortOrder: 1 },
      { id: 'tstg_2', key: 'IN_PROGRESS', label: 'In Progress', color: '#f59e0b', isDefault: false, sortOrder: 2 },
      { id: 'tstg_3', key: 'WAITING_ON_CUSTOMER', label: 'Waiting on Customer', color: '#8b5cf6', isDefault: false, sortOrder: 3 },
      { id: 'tstg_4', key: 'RESOLVED', label: 'Resolved', color: '#10b981', isDefault: false, sortOrder: 4 },
      { id: 'tstg_5', key: 'CLOSED', label: 'Closed', color: '#6b7280', isDefault: false, sortOrder: 5 },
    ],
    categories: [
...
export async function updateSection(workspaceId, sectionKey, data, userId = null) {
  ...
  if (sectionKey === 'lead_sources' && Array.isArray(data.sources)) {
    ...
    for (const src of data.sources) {
      if (!src.key) continue;
      const normalizedKey = String(src.key).trim().toUpperCase();
      if (seenKeys.has(normalizedKey)) {
        const error = new Error(`Duplicate lead source identifier: "${normalizedKey}". Source identifiers must be unique.`);
        error.status = 400;
        throw error;
      }
  ...
  // Ensure exactly one default stage for ticket customization
  if (sectionKey === 'ticket_customization' && Array.isArray(data.stages)) {
    ...
    } else if (defaultCount === 0 && data.stages.length > 0) {
      data.stages[0].isDefault = true;
    }
  }

  // Ensure exactly one default stage for lead lifecycle
  if (sectionKey === 'lead_lifecycle' && Array.isArray(data.stages)) {
    const defaultCount = data.stages.filter((s) => s.isDefault).length;
    if (defaultCount > 1) {
      let foundFirst = false;
      data.stages = data.stages.map((s) => {
...
export async function autoGenerateDealTaskOnStageChange(workspaceId, deal, newStage, userId = null) {
  try {
    ...
    const stageTemplates = autoTaskConfig.stageTaskTemplates || {};
    const configuredTemplate = stageTemplates[newStage];

    // Only create automatic tasks when a template is configured for the stage
    if (!configuredTemplate || !String(configuredTemplate).trim()) {
      return null;
    }
    ...
    const dealTitle = deal.title || deal.name || 'Deal';
    const taskTitle = String(configuredTemplate).trim()
      .replace(/\{dealTitle\}/g, dealTitle)
      .replace(/\{stage\}/g, newStage);

    // Prevent duplicate task creation for this deal and stage within a short window
    ...
    // Resolve valid assignee from deal owner, updating user, or null
    let validAssigneeId = null;
    const candidateId = deal.ownerUserId || userId;
    if (candidateId) {
      const u = await prisma.user.findUnique({ where: { id: candidateId }, select: { id: true } });
      if (u) validAssigneeId = u.id;
    }

    const priority = autoTaskConfig.defaultPriority || 'HIGH';

    const task = await prisma.task.create({
      data: {
        workspaceId,
        title: taskTitle,
        description: `[Priority: ${priority}] Auto-generated follow-up task triggered by stage transition to ${newStage}.`,
        dueDate,
...
/**
 * Helper to auto-generate a follow up Task when a Call or Visit/Meeting outcome is logged.
 * Handles Bug 7 (Call Outcome) and Bug 8 (Visit Outcome).
 */
export async function autoGenerateOutcomeTask(workspaceId, { type, outcome, sentiment, leadId, dealId, contactId, userId = null }) {
  try {
    if (!type || !outcome) return null;

    const normalizedType = String(type).toUpperCase();
    const sectionKey = (normalizedType === 'CALL') ? 'call_outcomes' : (normalizedType === 'MEETING') ? 'visit_outcomes' : null;
    ...
    if (!matchedOutcome || !matchedOutcome.triggersFollowUp) {
      return null;
    }

    const dueDate = new Date();
    dueDate.setDate(dueDate.getDate() + 1); // 1-day follow-up SLA

    // Resolve contact name for task title if possible
    let subjectName = 'Lead';
    if (contactId) {
      const contact = await prisma.contact.findFirst({ where: { id: contactId, workspaceId }, select: { name: true } });
      if (contact?.name) subjectName = contact.name;
    } else if (leadId) {
      const lead = await prisma.lead.findFirst({
        where: { id: leadId, workspaceId },
        include: { contact: { select: { name: true } } },
      });
      if (lead?.contact?.name) subjectName = lead.contact.name;
    }

    const isCall = (normalizedType === 'CALL');
    const taskTitle = isCall
      ? `Follow-up call: ${matchedOutcome.name} (${subjectName})`
      : `Mandatory follow-up: ${matchedOutcome.name} (${subjectName})`;

    // Check duplicate task created in the last 15 seconds
    ...
    const task = await prisma.task.create({
      data: {
        ...
        title: taskTitle,
        description: isCall
          ? `Prompted follow-up task on call outcome: "${matchedOutcome.name}". Sentiment: ${sentiment || matchedOutcome.sentiment || 'NEUTRAL'}.`
          : `Mandatory follow-up action triggered by meeting outcome: "${matchedOutcome.name}". Sentiment: ${sentiment || matchedOutcome.sentiment || 'NEUTRAL'}.`,
        dueDate,
        assignedToUserId: validAssigneeId,
        leadId: leadId || null,
        dealId: dealId || null,
    ...
    console.error('[autoGenerateOutcomeTask] Failed:', err.message);
    return null;
  }
}
