...
export async function getDeal(workspaceId, id, user = null) {
  ...
  const effectiveStage = deal.customFields?.stageKey || deal.stage;
  return { ...deal, stage: effectiveStage, health: await computeDealHealth(workspaceId, id) };
}
...
export async function updateDealStage(workspaceId, id, { stage, lostReason }, userId, user = null) {
  ...
  if (updated.previousStage !== stage) {
    ...
    autoGenerateDealTaskOnStageChange(workspaceId, updated, stage, userId)
      .catch((e) => console.error('[DealMode] auto-generate task error:', e.message));
  }
