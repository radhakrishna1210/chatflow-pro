...
import {
  ...
  deleteDefinition, validateCustomFields, listDefinitions,
...
test('unknown keys are rejected instead of being stored', async (t) => {
  if (!dbAvailable) return t.skip('database unavailable');
  await assert.rejects(
    () => validateCustomFields(workspaceId, 'lead', { not_a_field: 'x' }),
...
test('values are validated against their definition and merged over existing', async (t) => {
  ...
  const first = await validateCustomFields(workspaceId, 'lead', { budget: '5000' }, {});
  assert.equal(first.budget, 5000);

  // A later partial update must not wipe the earlier key.
  const merged = await validateCustomFields(workspaceId, 'lead', { segment: 'Mid' }, first);
  assert.deepEqual(merged, { budget: 5000, segment: 'Mid' });

  await assert.rejects(
    () => validateCustomFields(workspaceId, 'lead', { segment: 'Gigantic' }, merged),
    (e) => e.status === 400,
  );
});
...
test('a required field must be present after the merge, not just in the request', async (t) => {
  ...
  await assert.rejects(
    () => validateCustomFields(workspaceId, 'deal', {}, {}),
    (e) => e.status === 400 && /required/.test(e.message),
  );

  const ok = await validateCustomFields(workspaceId, 'deal', { approver: 'Priya' }, {});
  assert.equal(ok.approver, 'Priya');

  // Clearing it later must also fail.
  await assert.rejects(
    () => validateCustomFields(workspaceId, 'deal', { approver: '' }, ok),
    (e) => e.status === 400,
  );
...
test('a user field must name a member of this workspace', async (t) => {
  ...
  const ok = await validateCustomFields(workspaceId, 'lead', { reviewer: userId }, {});
  assert.equal(ok.reviewer, userId);

  await assert.rejects(
    () => validateCustomFields(workspaceId, 'lead', { reviewer: outsiderId }, {}),
...
test('passing null clears every custom field', async (t) => {
  if (!dbAvailable) return t.skip('database unavailable');
  assert.deepEqual(await validateCustomFields(workspaceId, 'lead', null, { budget: 1 }), {});
  // undefined means "not supplied" and leaves the record alone.
  assert.equal(await validateCustomFields(workspaceId, 'lead', undefined, { budget: 1 }), undefined);
});
