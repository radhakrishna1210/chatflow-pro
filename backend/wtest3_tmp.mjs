...
const PNID = num.metaPhoneNumberId, FROM = '919999000333';
...
const envl = (messages) => ({ object:'whatsapp_business_account', entry:[{ id:num.wabaId, changes:[{ field:'messages', value:{ ...meta, contacts, messages } }] }] });
const ts = Math.floor(Date.now()/1000), W = (s)=>`wamid.T3_${s}_${ts}`;
const msgs = [
...
console.log('batch delivery   ->', await post(envl(msgs)));
console.log('retry same batch ->', await post(envl(msgs)));
...
const stored = await p.message.findMany({ where:{ conversationId:convo.id, direction:'INBOUND' }, orderBy:{ createdAt:'asc' },
  select:{ body:true, type:true, status:true, mediaId:true, mediaMimeType:true, mediaFilename:true, locationLat:true } });
console.log('\nstored inbound messages:', stored.length, '(expect 5 — the retry must add none)');
